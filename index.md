...
围绕平台前端、测试基础设施和金融平台交付效率，负责公司级 **Foundation**（设计系统、Location 服务）与 **Treasury（资金管理）** 的迭代与治理。

#### Foundation（设计系统 / Location Service）

- 持续演进与维护含 **100+** 基础组件与业务组件的组件库，在一致性与可维护性前提下主导旧组件重构与废弃策略。
- 主导公司级组件库全面升级，推动 **20+** 个业务团队完成全量升级，系统性消除 **100+** 个 **major** 级兼容缺口。
- 自研自动化升级工具链，结合 AI 对公司前端仓库批量升级，将强依赖人工转为可规模复制的流程。
- 搭建组件 bundle size 等指标看板；维护 date-time 组件库，覆盖本地化与公司设计语义下的格式化。
- 主导 Location & IP（client-geolocation-service）迁移：补齐监控告警、规范域名路由、缩短请求耗时并补齐测试环境，**0 故障**全量切流；补齐自动化测试（**8** 个测试文件 / **39** 个用例全部通过）。
- 负责约 **70** 个微前端（MFE）迁至 **Vitest** 的盘点与推进，已完成 **9** 个模块迁移。
